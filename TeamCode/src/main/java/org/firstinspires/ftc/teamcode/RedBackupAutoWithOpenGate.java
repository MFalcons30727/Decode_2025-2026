...
@Disabled
@Autonomous
public class RedBackupAutoWithOpenGate extends OpMode {
        ...
        switch (currentAutoState){
            ...
            case EAT_TOP_RED_ARTIFACTS:
                if (!driver.isBusy() && !shooter.isShooting()){
                    ...
                    setAutoState(AutoState.TAP_OPEN_RED_GATE);
                }
                break;
            case TAP_OPEN_RED_GATE:
                if (!driver.isBusy() && !shooter.isShooting()){
                    driver.moveToPose(DriverDanny.Poses.RED_TAP_OPEN_GATE_POSE, true);
