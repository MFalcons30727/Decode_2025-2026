...
public class ShooterMcGavin {
    ...
    private double HOOD_SERVO_POSITION = 0; // the servo position of the adjustable hood
    ...
    private static final double TIME_TO_FEED_IN_MILLISECONDS = 1000; // this is how long it takes our indexers to feed one artifact through (the time between starting and stopping the indexers)
    ...
    private InterpLUT velocityLUT, hoodServoPositionLUT;
    ...
    public void update() { // THIS MUST ALWAYS GO IN YOUR OPMODE LOOP EVERY CALL
        switch (currentShootingState) {
            case START_SPIN_UP: // start the shooter
                ...
                hoodServo.setPosition(HOOD_SERVO_POSITION);
            ...
            case WAIT_FOR_TARGET_VELOCITY: // wait until we're close to the target velocity for the shooter
                if ((Math.abs(shootMotor.getVelocity() - SHOOTER_TARGET_VELOCITY) < SHOOTER_ACCEPTABLE_VELOCITY_ERROR && hoodServo.getPosition() == HOOD_SERVO_POSITION)
    ...
    public void startShootingFromDistance(double distanceFromGoalInInches) { // if this version of startShooting is called with no arguments, use the LUTs to determine velocity and hood servo position
        ...
        SHOOTER_TARGET_VELOCITY = velocityLUT.get(distanceFromGoalInInches);
        HOOD_SERVO_POSITION = hoodServoPositionLUT.get(distanceFromGoalInInches);
    ...
    public void startShootingAtVelocity(double targetVelocity) {
        ...
        HOOD_SERVO_POSITION = 0; // retract hood all the way
        setShootingState(ShootingState.START_SPIN_UP);
    }
}
